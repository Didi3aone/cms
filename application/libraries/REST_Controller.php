...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->input->is_ajax_request() === FALSE && $this->config->item('rest_ajax_only'))
        {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => 'NG',
                $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_ajax_only'),
                $this->config->item('rest_error_code_field_name') => 1
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->config->item('force_https') && $this->request->ssl === FALSE)
        {
            $this->response([
                ...
                $this->config->item('rest_error_code_field_name') => 2
            ], self::HTTP_FORBIDDEN);
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE)
        {
            ...
            $this->response([
                ...
                $this->config->item('rest_error_code_field_name') => 3
            ], self::HTTP_FORBIDDEN);
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && empty($this->rest->key) === FALSE && $this->_check_access() === FALSE)
        {
            ...
            $this->response([
                ...
                $this->config->item('rest_error_code_field_name') => 4
        ...
        if (! method_exists($this, $controller_method))
        {
            $this->response([
                ...
                $this->config->item('rest_error_code_field_name') => 5
        ...
        if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === FALSE)
        {
            ...
            {
                $response = [$this->config->item('rest_status_field_name') => 'NG', $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_time_limit'), $this->config->item('rest_error_code_field_name') => 6];
            ...
            if($authorized === FALSE)
            {
                ...
                $response = [$this->config->item('rest_status_field_name') => 'NG', $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_permissions'), $this->config->item('rest_error_code_field_name') => 7];
                $this->response($response, self::HTTP_UNAUTHORIZED);
        ...
        {
            $response = [$this->config->item('rest_status_field_name') => 'NG', $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_ip_address_time_limit'),$this->config->item('rest_error_code_field_name') => 6];
            $this->response($response, self::HTTP_UNAUTHORIZED);
    ...
    protected function _check_php_session()
    {
        ...
        if ( ! $this->session->userdata($key))
        {
            ...
            $this->response([
                ...
                $this->config->item('rest_error_code_field_name') => 9
            ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _prepare_digest_auth()
    {
        ...
        if (strcasecmp($digest['response'], $valid_response) !== 0)
        {
            ...
            $this->response([
                ...
                $this->config->item('rest_error_code_field_name') => 10
            ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _check_blacklist_auth()
    {
        ...
        if (preg_match($pattern, $this->config->item('rest_ip_blacklist')))
        {
            ...
            $this->response([
                ...
                $this->config->item('rest_error_code_field_name') => 11
            ], self::HTTP_UNAUTHORIZED);
        }
    }
